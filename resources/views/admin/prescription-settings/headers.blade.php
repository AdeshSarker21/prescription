...
@section('title', 'Prescription Headers')
...
@section('content')
    ...
    <div class="glass-card-static p-6 mb-6">
        <h2 class="text-lg font-semibold text-white/90 mb-4">Add New Header</h2>
        <form method="POST" action="{{ route('admin.prescription-settings.headers.store') }}">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-medium text-white/70 mb-1">Template Name</label>
                    <input type="text" name="name" required placeholder="e.g., Clinic Header - Main"
                        class="w-full glass-input @error('name') border-red-500 @enderror">
                    @error('name')<p class="text-red-400 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-white/70 mb-1">Header Content (HTML)</label>
                <textarea name="content" rows="6" required placeholder="<div class='header'>...</div>"
            ...
            <button type="submit" class="btn-gradient">Create Header</button>
        </form>
    </div>
                    ...
                    @forelse($items as $index => $item)
                        <tr class="border-b border-white/5 hover:bg-white/5">
                            ...
                            <td class="px-4 py-3 text-right space-x-2">
                                <button onclick="document.getElementById('edit-header-{{ $item->id }}').classList.toggle('hidden')" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 rounded-lg hover:bg-indigo-500/20 transition-colors">
                                    Edit
                                </button>
                                <form method="POST" action="{{ route('admin.prescription-settings.headers.destroy', $item->id) }}" class="inline" data-confirm="Delete this header template?" data-icon="warning">
                                    @csrf
                                    @method('DELETE')
                        ...
                        {{-- Edit Row --}}
                        <tr id="edit-header-{{ $item->id }}" class="hidden">
                            <td colspan="6" class="px-4 py-4 bg-white/5">
                                <form method="POST" action="{{ route('admin.prescription-settings.headers.update', $item->id) }}">
                                    @csrf
                                    @method('PATCH')
                                    ...
                                    <button type="submit" class="btn-gradient text-sm">Update Header</button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-12 text-white/40">No header templates found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
