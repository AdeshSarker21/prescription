...
<div class="space-y-6" x-data="serialDashboard()" x-init="init()">
    ...
    <div class="dashboard-card animate-card" style="border-left:4px solid #6366f1;">
        <div class="flex items-center gap-4 flex-wrap">
            ...
            @foreach($doctors as $doc)
                <a href="{{ route('assistant.smart-serial.dashboard', ['doctor_id' => $doc->id, 'chamber_id' => $activeChamberId]) }}"
    ...
    @elseif($doctors->count() === 1)
        @if(!$selectedDoctorId)
            <div class="dashboard-card animate-card" style="border-left:4px solid #6366f1;">
                <form method="POST" action="{{ route('assistant.smart-serial.set-doctor') }}" class="flex items-center gap-4">
                    @csrf
                    <input type="hidden" name="doctor_id" value="{{ $doctors->first()->id }}">
                    <span class="text-sm" style="color:var(--text-muted);">Managing: <strong>{{ $doctors->first()->name }}</strong></span>
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">Select</button>
                </form>
            </div>
        @endif
    @endif
    ...
    <div class="flex items-center gap-2 flex-wrap">
        ...
        @foreach($chambers as $chamber)
            <a href="{{ route('assistant.smart-serial.dashboard', ['doctor_id' => $selectedDoctorId, 'chamber_id' => $chamber->id]) }}"
    ...
    <div class="dashboard-card animate-card" style="border-left:4px solid #6366f1;">
        <div class="flex items-center justify-between flex-wrap gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('assistant.smart-serial.display.doctor', $session->doctor_id) }}" target="_blank" class="text-sm font-medium transition-all" style="color:#059669;padding:6px 14px;border-radius:8px;background:rgba(16,185,129,0.08);">&#128250; Patient Display</a>
                <a href="{{ route('assistant.smart-serial.index', ['doctor_id' => $selectedDoctorId]) }}" class="text-sm font-medium transition-all" style="color:#6366f1;padding:6px 14px;border-radius:8px;background:rgba(99,102,241,0.08);">Open Queue &rarr;</a>
            </div>
        </div>
    </div>
    ...
    <style>
        ...
        .voice-bar .status-dot { width:8px; height:8px; border-radius:50%; background:#64748b; transition:all 0.3s; }
        .voice-bar .status-dot.active { background:#22c55e; box-shadow:0 0 8px rgba(34,197,94,0.5); }
        .voice-bar .enable-btn { background:rgba(99,102,241,0.9); color:#fff; border:none; padding:8px 18px; border-radius:8px; font-size:0.85rem; font-weight:600; cursor:pointer; transition:all 0.2s; }
        .voice-bar .enable-btn:hover { background:rgba(99,102,241,1); }
        .voice-bar .label { font-size:0.8rem; color:var(--text-muted); }
    </style>
    <div class="dashboard-card animate-card" style="border-left:4px solid #8b5cf6;">
        <div class="voice-bar">
            <button class="enable-btn" @click="toggleVoice()" x-text="speechReady ? 'Voice ON' : 'Voice OFF'" :style="speechReady ? 'background:rgba(34,197,94,0.9);' : ''"></button>
        ...
        <div class="dashboard-card animate-card" style="border-left:4px solid #06b6d4;">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="stat-value" style="color:#0891b2;margin-top:4px;">{{ $avgWaitMinutes > 0 ? $avgWaitMinutes . 'm' : '—' }}</p>
                </div>
    ...
    {{-- Now Calling + Current/Next Patient --}}
        ...
        <div class="dashboard-card animate-card" style="border-top:3px solid #8b5cf6;">
            @if($nextPatient)
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <p class="text-sm font-bold" style="color:var(--text-primary);">Next in Queue</p>
                </div>
                <p class="text-lg font-bold" style="color:#7c3aed;">#{{ $nextPatient->formatted_serial }} - {{ $nextPatient->patient->name ?? 'N/A' }}</p>
                <p class="text-xs mt-1" style="color:var(--text-muted);">{{ ucfirst($nextPatient->priority) }} priority</p>
            @else
                <div class="flex items-center gap-3 mb-3">
                    <div class="stat-icon" style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,rgba(107,114,128,0.1),rgba(107,114,128,0.05));color:#9ca3af;">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                    </div>
                    <p class="text-sm font-bold" style="color:var(--text-primary);">Next in Queue</p>
                </div>
                <p class="text-sm" style="color:var(--text-muted);">No patients waiting</p>
            @endif
        </div>
    </div>
        ...
        <div class="flex items-center justify-between flex-wrap gap-3">
            ...
            <div class="flex gap-2 flex-wrap">
                ...
                @if($currentCalled)
                    ...
                    <form method="POST" action="{{ route('assistant.smart-serial.complete', $currentCalled->id) }}">
                        @csrf @method('PATCH')
                    ...
                    <form method="POST" action="{{ route('assistant.smart-serial.recall', $currentCalled->id) }}">
                        @csrf @method('PATCH')
                    ...
                    <form method="POST" action="{{ route('assistant.smart-serial.skip', $currentCalled->id) }}">
                        @csrf @method('PATCH')
    ...
    <div class="dashboard-card animate-card p-12 text-center">
        ...
        <a href="{{ route('assistant.smart-serial.index', ['doctor_id' => $selectedDoctorId]) }}" class="btn-gradient inline-block mt-6">Go to Smart Serial Queue</a>
    </div>
    @endif
...
function serialDashboard() {
    return {
        ...
        voiceEnabled: @js($voiceEnabled),
        refreshTimer: null,
        speechReady: false,
        voiceStatusText: 'Click to enable voice',
        ttsQueue: [],
        ttsPlaying: false,
        ttsCache: {},
        lastPendingKey: null,
        ...
        toggleVoice() {
            if (this.speechReady) {
                ...
                this.voiceStatusText = 'Voice Disabled';
            } else {
                this.enableVoice();
            }
        },
        ...
        async refreshQueue() {
            ...
            try {
                ...
                if (this.voiceEnabled && data.pending_announcement && data.pending_queue_id && data.pending_voice_text) {
