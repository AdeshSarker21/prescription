...
@section('content')
<div class="max-w-2xl">
    <form action="{{ route('doctor.notices.update', $notice) }}" method="POST" class="space-y-6">
        @csrf
        @method('PATCH')
        ...
        <div>
            ...
            <textarea name="message" id="message" rows="4" required
                class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('message', $notice->message) }}</textarea>
            @error('message') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <div class="flex items-center gap-3">
            <input type="hidden" name="is_active" value="0">
            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $notice->is_active) ? 'checked' : '' }}
