...
<form method="POST" action="{{ route('doctor.prescriptions.store') }}" id="prescription-form">
    ...
    <section class="patient-info-grid">
        <div class="form-group" style="grid-column:span 2;">
            ...
            <button type="button" onclick="openLoadModal()" class="btn-secondary" style="padding:6px 12px;font-size:12px;border:none;border-radius:8px;cursor:pointer;font-weight:600;white-space:nowrap;display:none;" id="view-history-btn">
        ...
        <div class="form-group">
            <label>Date:</label>
            <input type="text" value="{{ \Carbon\Carbon::now('Asia/Dhaka')->format('d/m/Y') }}" readonly>
        </div>

        <div class="form-group">
            <label>Prescription No:</label>
            <input type="text" value="Auto" class="w-small" readonly>
        </div>
        <div class="form-group">
            <label>Time:</label>
            <input type="text" value="{{ \Carbon\Carbon::now('Asia/Dhaka')->format('g:i A') }}" class="w-medium" readonly>
        </div>
        ...
        <div class="form-group span-2">
            ...
            <input type="text" name="guardian_name" class="w-full" value="{{ old('guardian_name') }}" placeholder="Father or guardian name">
        </div>
        ...
        <div class="vitals-group" style="grid-column:span 4;">
            <div class="form-group inline-vitals">
                <label>BP:</label>
                <input type="text" name="bp_systolic" value="{{ old('bp_systolic') }}" class="w-vitals" placeholder="120">
                <span style="color:var(--text-muted);font-size:14px;">/</span>
                <input type="text" name="bp_diastolic" value="{{ old('bp_diastolic') }}" class="w-vitals" placeholder="80">
                <span class="unit">mmHg</span>
            </div>
            <div class="form-group inline-vitals">
                <label>Pulse Rate:</label>
                <input type="text" name="pulse_rate" value="{{ old('pulse_rate') }}" class="w-vitals" placeholder="72">
                <span class="unit">/min</span>
            </div>
            <div class="form-group inline-vitals">
                <label>SpO₂:</label>
                <input type="text" name="spo2" value="{{ old('spo2') }}" class="w-vitals" placeholder="98" min="0" max="100" step="0.1">
                <span class="unit">%</span>
            </div>
            <div class="form-group inline-vitals">
                <label>Ht (cm):</label>
                <input type="text" name="height" id="rx-height" value="{{ old('height', 0) }}" class="w-vitals" placeholder="170" step="0.1" min="0" max="300">
                <span id="rx-height-prev" class="prev-val"></span>
            </div>
            <div class="form-group inline-vitals">
                <label>Wt (kg):</label>
                <input type="text" name="weight" id="rx-weight" value="{{ old('weight', 0) }}" class="w-vitals" placeholder="70" step="0.01" min="0" max="500">
    ...
    <div class="prescription-body">
        <div class="left-pane">
            ...
            <div class="glass-card pane-section">
                <h3>D D</h3>
                <textarea name="diagnosis" rows="3" class="w-full" style="padding:8px;border:1px solid rgba(148,163,184,0.3);border-radius:8px;background:rgba(255,255,255,0.6);font-size:13px;outline:none;width:100%;resize:vertical;">{{ old('diagnosis') }}</textarea>
            </div>
        </div>
        ...
        <div style="display:flex;flex-direction:column;" x-data="prescriptionItems()">
            ...
            <div class="submit-bar">
                ...
                <button type="submit" class="btn-primary" style="padding:8px 20px;font-size:13px;font-weight:600;border:none;border-radius:10px;cursor:pointer;">Save Prescription</button>
                <button type="button" class="btn-success" style="padding:8px 20px;font-size:13px;font-weight:600;border:none;border-radius:10px;cursor:pointer;" onclick="saveAndAddMedicinesLater()">Save & Add Medicines Later</button>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:16px;">
                ...
                <div class="glass-card advice-section">
                    <h3>Follow-up</h3>
                    <textarea name="follow_up_instructions" rows="2" class="w-full" style="padding:8px;border:1px solid rgba(148,163,184,0.3);border-radius:8px;background:rgba(255,255,255,0.6);font-size:13px;outline:none;transition:all 0.2s;">{{ old('follow_up_instructions') }}</textarea>
                    <div style="margin-top:8px;display:flex;gap:12px;align-items:center;">
                        <label style="font-size:13px;font-weight:600;color:var(--text-primary);">Follow-up Date:</label>
                        <input type="date" name="follow_up_date" value="{{ old('follow_up_date') }}" style="padding:6px 12px;border:1px solid rgba(148,163,184,0.3);border-radius:8px;background:rgba(255,255,255,0.6);font-size:13px;outline:none;transition:all 0.2s;">
                    </div>
                </div>
            </div>
...
<script>
    window.previousPrescriptions = @json($previousPrescriptionsJson);
</script>

{{-- Previous Prescription History Panel --}}
<div id="previous-rx-panel" style="display:none;margin-top:12px;">
    <div class="glass-card" style="padding:12px;border-left:4px solid var(--primary);">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            ...
            <div style="display:flex;gap:6px;">
                <button type="button" class="preset-btn" onclick="toggleHistoryPanel()" style="font-size:11px;">Show All History</button>
                <button type="button" class="preset-btn" onclick="closeHistoryPanel()" style="font-size:11px;">Hide</button>
            </div>
        </div>
        <div id="previous-rx-list" style="display:flex;flex-direction:column;gap:8px;max-height:300px;overflow-y:auto;"></div>
    </div>
</div>

{{-- Load Previous Prescription Modal --}}
<div id="load-prescription-modal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;z-index:3000;align-items:center;justify-content:center;background:rgba(0,0,0,0.3);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);">
    <div style="background:rgba(255,255,255,0.75);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border-radius:16px;padding:28px;width:680px;max-width:95vw;max-height:90vh;overflow-y:auto;border:1px solid rgba(255,255,255,0.5);box-shadow:0 25px 60px rgba(0,0,0,0.15);animation:fadeInUp 0.3s ease-out;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
            <h3 style="font-size:18px;font-weight:700;color:var(--text-primary);">Load Previous Prescription</h3>
            <button type="button" onclick="closeLoadModal()" style="background:rgba(148,163,184,0.15);border:none;font-size:24px;cursor:pointer;color:var(--text-muted);padding:4px 10px;border-radius:8px;">&times;</button>
        </div>
        <div id="load-prescription-list" style="max-height:500px;overflow-y:auto;"></div>
        <div id="copy-selection-section" style="display:none;margin-top:16px;border-top:1px solid rgba(148,163,184,0.2);padding-top:16px;">
            <h4 style="font-size:14px;font-weight:700;margin-bottom:10px;">Select Data to Copy</h4>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
                <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;padding:6px 10px;background:rgba(99,102,241,0.06);border-radius:8px;border:1px solid rgba(99,102,241,0.1);">
                    <input type="checkbox" id="copy-complaints" checked> <span>Complaints</span>
                </label>
                <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;padding:6px 10px;background:rgba(99,102,241,0.06);border-radius:8px;border:1px solid rgba(99,102,241,0.1);">
                    <input type="checkbox" id="copy-tests" checked> <span>Investigations</span>
                </label>
                <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;padding:6px 10px;background:rgba(99,102,241,0.06);border-radius:8px;border:1px solid rgba(99,102,241,0.1);">
                    <input type="checkbox" id="copy-diagnosis" checked> <span>Diagnosis</span>
                </label>
                <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;padding:6px 10px;background:rgba(99,102,241,0.06);border-radius:8px;border:1px solid rgba(99,102,241,0.1);">
                    <input type="checkbox" id="copy-medicines" checked> <span>Medicines</span>
                </label>
                <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;padding:6px 10px;background:rgba(99,102,241,0.06);border-radius:8px;border:1px solid rgba(99,102,241,0.1);">
                    <input type="checkbox" id="copy-advice" checked> <span>Advice</span>
                </label>
                <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;padding:6px 10px;background:rgba(99,102,241,0.06);border-radius:8px;border:1px solid rgba(99,102,241,0.1);">
                    <input type="checkbox" id="copy-followup" checked> <span>Follow-up Date</span>
                </label>
                <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;padding:6px 10px;background:rgba(99,102,241,0.06);border-radius:8px;border:1px solid rgba(99,102,241,0.1);">
                    <input type="checkbox" id="copy-vitals" checked> <span>Vitals</span>
                </label>
            </div>
            <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:16px;">
                <button type="button" onclick="closeLoadModal()" class="btn-secondary" style="padding:10px 20px;font-size:13px;font-weight:600;border:none;border-radius:8px;cursor:pointer;">Cancel</button>
                <button type="button" id="copy-selected-btn" onclick="copySelectedData()" class="btn-primary" style="padding:10px 20px;font-size:13px;font-weight:600;border:none;border-radius:8px;cursor:pointer;opacity:0.5;" disabled>Copy Selected Data</button>
            </div>
        </div>
    </div>
</div>
...
<button type="button" class="btn-action btn-success" onclick="openLoadModal()" id="sidebar-history-btn" style="display:none;">
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
    Load Previous
</button>
<button type="button" class="btn-action btn-warning" onclick="if(confirm('Delete this prescription?')){window.location.href='{{ route('doctor.prescriptions.index') }}'}">
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
    Delete
</button>
<button type="button" class="btn-action btn-danger" onclick="window.location.href='{{ route('doctor.prescriptions.index') }}'">
...
<button type="button" class="btn-action btn-success" onclick="window.location.href='{{ route('doctor.prescriptions.index') }}'">
...
<script>
    ...
    function prescriptionItems() {
        return {
            items: [{ type: 'medicine', medicine_name: '', medicine_id: '', strength: '', category_name: '', frequency: '', duration: '', instructions: '', sort_order: 0, seal_id: null, seal_text: '', seal_details: '' }],
            nextSortOrder: 1,
    ...
    function featureMenstrualHistory() {
        return {
            data: { lmp: '', cycle: '', duration: '', flow: '', notes: '' },
            init() {
                var self = this;
    ...
    function getPrescriptionsForPatient(patientId) {
        const data = window.previousPrescriptions || [];
        const found = data.find(p => p.patient_id === parseInt(patientId));
    ...
    function populatePatientData() {
        ...
        const panel = document.getElementById('previous-rx-panel');
        if (pid) {
            const rxList = getPrescriptionsForPatient(pid);
            if (rxList.length > 0) {
                historyBtn.style.display = 'inline-flex';
                sidebarHistoryBtn.style.display = 'flex';
                renderHistoryPanel(pid);
                panel.style.display = 'block';
            } else {
                historyBtn.style.display = 'none';
                sidebarHistoryBtn.style.display = 'none';
    ...
    function renderHistoryPanel(patientId) {
        const list = getPrescriptionsForPatient(patientId);
        const container = document.getElementById('previous-rx-list');
        if (!list.length) {
            container.innerHTML = '<div style="color:#999;font-size:12px;padding:8px;">No previous prescriptions found.</div>';
            return;
        }
        container.innerHTML = list.map(function(rx) {
            const statusClass = rx.status === 'investigation_pending' ? 'bg-amber-100 text-amber-700' :
                rx.status === 'treatment_started' ? 'bg-green-100 text-green-700' :
                rx.status === 'completed' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600';
            const items = rx.items || [];
            const complaints = rx.complaints || [];
            const tests = rx.tests || [];
            const advice = rx.advice || [];
            return '<div style="background:rgba(255,255,255,0.5);border-radius:8px;padding:10px 12px;border:1px solid rgba(148,163,184,0.15);">' +
                '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">' +
                '<span style="font-size:12px;font-weight:700;color:var(--primary);">' + rx.date + ' - ' + (rx.number || 'RX') + '</span>' +
                '<span class="px-2 py-0.5 rounded-full text-xs font-semibold ' + statusClass + '" style="font-size:10px;padding:1px 8px;border-radius:10px;">' +
                rx.status.replace(/_/g, ' ') +
                '</span></div>' +
                (rx.diagnosis ? '<div style="font-size:11px;color:#333;margin-bottom:3px;"><strong>Dx:</strong> ' + rx.diagnosis + '</div>' : '') +
                (complaints.length ? '<div style="font-size:11px;color:#555;margin-bottom:2px;">' + complaints.slice(0, 3).map(function(c) { return '<span style="display:inline-block;padding:0 6px;background:#e0f0ff;border-radius:4px;margin:1px;">' + c.name + '</span>'; }).join('') + (complaints.length > 3 ? ' <span style="color:#999;">+' + (complaints.length - 3) + '</span>' : '') + '</div>' : '') +
                (tests.length ? '<div style="font-size:10px;color:#888;margin-bottom:2px;"><strong>Tests:</strong> ' + tests.map(function(t) {
                    var tr = rx.testReportResults && rx.testReportResults.find(function(r) { return r.test_name.toLowerCase() === t.name.toLowerCase(); });
                    return t.name + (tr && tr.result ? ' (' + tr.result + ')' : '');
                }).join(', ') + '</div>' : '') +
                (items.length ? '<div style="font-size:10px;color:#888;"><strong>Rx:</strong> ' + items.slice(0, 2).map(function(i) { return i.medicine_name; }).join(', ') + (items.length > 2 ? ' +' + (items.length - 2) : '') + '</div>' : '<div style="font-size:10px;color:#aaa;font-style:italic;">No medicines prescribed</div>') +
                '<div style="margin-top:6px;display:flex;gap:4px;">' +
                '<button type="button" class="preset-btn" style="font-size:10px;padding:2px 10px;" onclick="openLoadModal()">Load & Copy</button>' +
                '</div></div>';
        }).join('');
    }
    ...
    function openLoadModal() {
        ...
        const list = getPrescriptionsForPatient(pid);
        const container = document.getElementById('load-prescription-list');
        const copyBtn = document.getElementById('copy-selected-btn');
        selectedPrescriptionId = null;
        copyBtn.disabled = true;
        copyBtn.style.opacity = '0.5';
        if (!list.length) {
            container.innerHTML = '<div style="text-align:center;padding:30px;color:#999;">No previous prescriptions for this patient.</div>';
            document.getElementById('copy-selection-section').style.display = 'none';
        } else {
            container.innerHTML = list.map(function(rx) {
                const statusClass = rx.status === 'investigation_pending' ? 'bg-amber-100 text-amber-700' :
                    rx.status === 'treatment_started' ? 'bg-green-100 text-green-700' :
                ...
                const tests = rx.tests || [];
                const advice = rx.advice || [];
                return '<div class="rx-card" data-rx-id="' + rx.id + '" style="background:rgba(255,255,255,0.5);border-radius:10px;padding:14px;margin-bottom:10px;border:1px solid rgba(148,163,184,0.15);cursor:pointer;transition:all 0.2s;">' +
                    '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">' +
                    '<div><span style="font-weight:700;font-size:14px;color:var(--primary);">' + rx.date + '</span>' +
                    '<span style="color:#999;font-size:11px;margin-left:8px;">#' + (rx.number || '') + '</span></div>' +
                    '<span class="px-2 py-0.5 rounded-full text-xs font-semibold ' + statusClass + '" style="font-size:10px;padding:2px 10px;border-radius:10px;">' +
                    rx.status.replace(/_/g, ' ') +
                    '</span></div>' +
                    (rx.diagnosis ? '<div style="font-size:12px;color:#333;margin-bottom:4px;"><strong>Diagnosis:</strong> ' + rx.diagnosis + '</div>' : '') +
                    '<div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:4px;">' +
                    (complaints.length ? '<span style="font-size:11px;color:#666;margin-right:4px;"><strong>Complaints:</strong></span> ' + complaints.slice(0, 4).map(function(c) { return '<span style="display:inline-block;padding:0 6px;background:#e0f0ff;border-radius:4px;font-size:10px;">' + c.name + '</span>'; }).join('') + (complaints.length > 4 ? ' <span style="color:#999;font-size:10px;">+' + (complaints.length - 4) + '</span>' : '') : '') +
                    '</div>' +
                    (tests.length ? '<div style="font-size:11px;color:#666;margin-bottom:4px;"><strong>Tests:</strong> ' + tests.map(function(t) {
                        var testResult = rx.testReportResults && rx.testReportResults.find(function(r) { return r.test_name.toLowerCase() === t.name.toLowerCase(); });
                        var resultStr = testResult && testResult.result ? ' (' + testResult.result + ')' : '';
                        return '<span style="display:inline-block;padding:0 6px;background:#fff3cd;border-radius:4px;font-size:10px;">' + t.name + resultStr + '</span>';
                    }).join('') + '</div>' : '') +
                    (items.length ? '<div style="font-size:11px;color:#666;"><strong>Medicines:</strong> ' + items.slice(0, 3).map(function(i) { return i.type === 'seal' ? (i.seal_text || 'Clinical Seal') : i.medicine_name; }).join(', ') + (items.length > 3 ? ' <span style="color:#999;">+' + (items.length - 3) + '</span>' : '') + '</div>' : '<div style="font-size:11px;color:#aaa;font-style:italic;">No medicines prescribed</div>') +
                    '<div style="margin-top:8px;display:flex;gap:4px;flex-wrap:wrap;">' +
                    advice.slice(0, 3).map(function(a) { return '<span style="display:inline-block;padding:1px 8px;background:#e0f0ff;border:1px solid #71a9ce;border-radius:10px;font-size:10px;">' + a + '</span>'; }).join('') +
                    '</div>' +
                    '</div>';
            }).join('');
            document.getElementById('copy-selection-section').style.display = 'block';
        }
        document.getElementById('load-prescription-modal').style.display = 'flex';
    }

    function selectPrescription(rxId, el) {
        selectedPrescriptionId = rxId;
        $('#load-prescription-list .rx-card').removeClass('rx-card-active')
            .css({ borderColor: 'rgba(148,163,184,0.15)', boxShadow: 'none', background: 'rgba(255,255,255,0.5)' });
        const $card = el ? $(el).closest('.rx-card') : $('#load-prescription-list .rx-card[data-rx-id="' + rxId + '"]');
        $card.addClass('rx-card-active')
            .css({ borderColor: 'var(--primary)', boxShadow: '0 0 0 2px rgba(99,102,241,0.2)', background: 'rgba(99,102,241,0.04)' });
        const copyBtn = document.getElementById('copy-selected-btn');
        if (copyBtn) {
    ...
    $(document).on('click', '#load-prescription-list .rx-card', function() {
        const rxId = parseInt($(this).data('rx-id'));
        selectPrescription(rxId, this);
    });

    function closeLoadModal() {
        document.getElementById('load-prescription-modal').style.display = 'none';
        selectedPrescriptionId = null;
    }

    function copySelectedData() {
        if (!selectedPrescriptionId) {
            Swal.fire({ icon: 'warning', title: 'Select a Prescription', text: 'Please click on a prescription to select it first.' });
            return;
        }
        const sel = document.getElementById('patient-select');
        const pid = sel.value;
        const list = getPrescriptionsForPatient(pid);
        const rx = list.find(function(r) { return r.id === selectedPrescriptionId; });
        if (!rx) return;

        const copyComplaints = document.getElementById('copy-complaints').checked;
        const copyTests = document.getElementById('copy-tests').checked;
        const copyDiagnosis = document.getElementById('copy-diagnosis').checked;
        const copyMedicines = document.getElementById('copy-medicines').checked;
        const copyAdvice = document.getElementById('copy-advice').checked;
        const copyFollowup = document.getElementById('copy-followup').checked;
        const copyVitals = document.getElementById('copy-vitals').checked;

        if (!copyComplaints && !copyTests && !copyDiagnosis && !copyMedicines && !copyAdvice && !copyFollowup && !copyVitals) {
            Swal.fire({ icon: 'warning', title: 'Nothing Selected', text: 'Please select at least one item to copy.' });
            return;
        }

        if (copyDiagnosis && rx.diagnosis) {
            const diagField = document.querySelector('[name="diagnosis"]');
            if (diagField && !diagField.value) {
                diagField.value = rx.diagnosis;
            }
        }

        if (copyComplaints && rx.complaints && rx.complaints.length) {
            const el = document.getElementById('complaints-section');
            if (el && window.Alpine) {
                const data = window.Alpine.$data(el);
                rx.complaints.forEach(function(c) {
                    if (!data.selected.find(function(ex) { return ex.name.toLowerCase() === c.name.toLowerCase(); })) {
                        data.selected.push({ id: null, name: c.name, notes: c.notes || '' });
                    }
                });
            }
        }

        if (copyTests && rx.tests && rx.tests.length) {
            const el = document.getElementById('tests-section');
            if (el && window.Alpine) {
                const data = window.Alpine.$data(el);
                rx.tests.forEach(function(t) {
                    if (!data.selected.find(function(ex) { return ex.name.toLowerCase() === t.name.toLowerCase(); })) {
                        const result = rx.testReportResults && rx.testReportResults.find(function(r) { return r.test_name.toLowerCase() === t.name.toLowerCase(); });
                        data.selected.push({ id: null, name: t.name, result: result ? result.result : '' });
                    }
                });
            }
        }

        if (copyMedicines && rx.items && rx.items.length) {
            const medEl = document.querySelector('[x-data="prescriptionItems()"]');
            if (medEl && window.Alpine) {
                const data = window.Alpine.$data(medEl);
                rx.items.forEach(function(item) {
                    const isSeal = item.type === 'seal';
                    const exists = data.items.some(function(ex) {
                        if (isSeal) {
                            return ex.type === 'seal' && ex.seal_text === item.seal_text;
                        }
                        return (ex.medicine_name || '').toLowerCase() === (item.medicine_name || '').toLowerCase();
                    });
                    if (exists) return;
                    data.items.push({
                        type: isSeal ? 'seal' : 'medicine',
                        ...
                        seal_details: item.seal_details || '',
        ...
        if (copyAdvice && rx.advice && rx.advice.length) {
            const advEl = document.querySelector('[x-data="modalAdvice()"]');
            if (advEl && window.Alpine) {
                const data = window.Alpine.$data(advEl);
                rx.advice.forEach(function(a) {
                    if (a && !data.selected.find(function(ex) { return ex.name && ex.name.toLowerCase() === a.toLowerCase(); })) {
                        data.selected.push({ id: null, name: a });
                    }
                });
            }
        }

        if (copyFollowup && rx.follow_up_date) {
            const fuField = document.querySelector('[name="follow_up_date"]');
            if (fuField && !fuField.value) {
                fuField.value = rx.follow_up_date;
            }
            if (rx.follow_up_instructions) {
                const fiField = document.querySelector('[name="follow_up_instructions"]');
                if (fiField && !fiField.value) {
                    fiField.value = rx.follow_up_instructions;
                }
            }
        }
        ...
        if (copyVitals) {
            ...
            Object.keys(vitalsMap).forEach(function(key) {
                if (rx[key] !== null && rx[key] !== undefined && rx[key] !== '') {
            ...
            if (rx.weight && rx.weight > 0 || rx.height && rx.height > 0) {
                document.getElementById('rx-weight')?.dispatchEvent(new Event('input'));
                document.getElementById('rx-height')?.dispatchEvent(new Event('input'));
            }
        }
        ...
        Swal.fire({
            icon: 'success',
            title: 'Data Copied',
            text: 'Selected data from previous prescription has been added to this form.',
            timer: 2000,
            showConfirmButton: false
        });
    }
    ...
    function closeHistoryPanel() {
        document.getElementById('previous-rx-panel').style.display = 'none';
    }
