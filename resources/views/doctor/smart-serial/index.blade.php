...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 mt-1">Manage your patient queue for today</p>
        </div>
        @if($session && $session->status === 'active')
            <div class="flex gap-2">
                <form method="POST" action="{{ route('doctor.smart-serial.pause', $session->id) }}">
                    @csrf @method('PATCH')
                    <button type="submit" class="px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600">Pause</button>
                </form>
                <form method="POST" action="{{ route('doctor.smart-serial.close', $session->id) }}">
                    @csrf @method('PATCH')
                    <button type="submit" class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">Close Session</button>
                </form>
            </div>
        @elseif($session && $session->status === 'paused')
            <form method="POST" action="{{ route('doctor.smart-serial.resume', $session->id) }}">
                @csrf @method('PATCH')
    ...
    @if($session)
        ...
        <div class="bg-white rounded-xl shadow p-4">
            ...
            <form method="POST" action="{{ route('doctor.smart-serial.add-patient') }}" class="flex gap-3 items-end">
                @csrf
                <div class="flex-1">
                ...
                <div>
                    ...
                    <input type="number" name="appointment_id" class="mt-1 block w-full border rounded-lg px-3 py-2" placeholder="Optional">
                </div>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex gap-1 flex-wrap">
                                    @if($item->status === 'waiting' && in_array('prepare', $permissions))
                                        <form method="POST" action="{{ route('doctor.smart-serial.prepare-patient', $item->id) }}">
                                            @csrf @method('PATCH')
                                    ...
                                    @if($item->status === 'waiting' && in_array('call_next', $permissions))
                                        <form method="POST" action="{{ route('doctor.smart-serial.call-patient', $item->id) }}">
                                            @csrf @method('PATCH')
                                    ...
                                    @if(!in_array($item->status, ['cancelled','skipped']))
                                        <a href="{{ route('doctor.smart-serial.print-token', $item->id) }}" target="_blank" class="px-2 py-1 bg-teal-500 text-white rounded text-xs hover:bg-teal-600 inline-block" title="Print Token">&#128424;</a>
                                    @endif
                                </div>
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="6" class="px-4 py-8 text-center text-gray-400">No patients in queue</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
