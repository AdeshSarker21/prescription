...
<div class="max-w-3xl mx-auto space-y-6" x-data="addSerialApp()">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('doctor.smart-serial.index', $activeChamberId ? ['chamber_id' => $activeChamberId] : []) }}" class="btn-gradient text-sm">&larr; Back to Queue</a>
    </div>
    ...
    @if(!$session)
        <div class="dashboard-card animate-card p-12 text-center" style="border-left:4px solid #f59e0b;">
            ...
            <a href="{{ route('doctor.smart-serial.index') }}" class="btn-gradient inline-block mt-6">Start Session</a>
        </div>
    @else
        <form method="POST" action="{{ route('doctor.smart-serial.add-patient') }}">
            @csrf
                ...
                <div class="flex gap-3">
                    <div class="relative flex-1">
                        ...
                        <div x-show="showDropdown && results.length > 0" x-cloak @click.away="showDropdown = false"
                             class="absolute z-[999] w-full mt-1 glass-dropdown">
                            <div class="glass-dropdown-scroll">
                                <template x-for="patient in results" :key="patient.id">
                                    <div class="px-4 py-3 cursor-pointer transition-all border-b border-white/20 last:border-b-0 hover:bg-white/40" @click="selectPatient(patient)">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                ...
                <div class="dashboard-card animate-card" style="border-left:4px solid #8b5cf6;">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @else
                            <select name="chamber_id" class="border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500"
                                style="border-color:rgba(148,163,184,0.2);background:rgba(255,255,255,0.7);color:var(--text-primary);">
            ...
            <div class="flex justify-end gap-3 pt-2">
                <a href="{{ route('doctor.smart-serial.index', $activeChamberId ? ['chamber_id' => $activeChamberId] : []) }}"
                ...
                <button type="submit" :disabled="!selectedPatient"
                    class="px-8 py-2.5 rounded-lg text-sm font-bold text-white transition-all px-2"
